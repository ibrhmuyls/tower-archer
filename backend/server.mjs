  ...
  try {
    if (req.method === 'POST' && pathname === '/api/upgrade') {
      ...
      if (!address || !upgradeType || Number.isNaN(level) || Number.isNaN(cost)) {
        res.writeHead(400, json({ error: 'address, upgradeType, level, cost required' }).headers);
        res.end(JSON.stringify({ error: 'address, upgradeType, level, cost required' }));
        return;
      }
      ...
      account.spent += cost;
      account.upgrades[`${upgradeType}#${level}`] = {
        upgradeType,
        level,
        cost,
        timestamp: Date.now()
      ...
      res.writeHead(200, json({ ok: true, spent: account.spent, upgrade: account.upgrades[`${upgradeType}#${level}`] }).headers);
      res.end(JSON.stringify({ ok: true, spent: account.spent, upgrade: account.upgrades[`${upgradeType}#${level}`] }));
      return;
    }
    ...
    if (req.method === 'POST' && pathname === '/api/sync') {
      ...
      if (!address || !address.startsWith('0x')) {
        res.writeHead(400, json({ error: 'valid address required' }).headers);
      ...
      res.writeHead(200, json({ address, baseBalance: realBalance, spent: account.spent, available, upgrades: account.upgrades }).headers);
      res.end(JSON.stringify({ address, baseBalance: realBalance, spent: account.spent, available, upgrades: account.upgrades }));
      return;
    }
    ...
    if (req.method === 'GET' && pathname.startsWith('/api/balance/')) {
      ...
      res.writeHead(200, json({ address, baseBalance: account.baseBalance, spent: account.spent, available, upgrades: account.upgrades }).headers);
      res.end(JSON.stringify({ address, baseBalance: account.baseBalance, spent: account.spent, available, upgrades: account.upgrades }));
      return;
    }

    res.writeHead(404, json({ error: 'Not Found' }).headers);
    res.end(JSON.stringify({ error: 'Not Found' }));
  } catch (err) {
    console.error('Request error:', err);
    res.writeHead(500, json({ error: 'Internal Server Error' }).headers);
    res.end(JSON.stringify({ error: 'Internal Server Error' }));
  }
});

server.listen(PORT, HOST, () => {
  console.log(`Tower Archer backend running on http://${HOST}:${PORT}`);
